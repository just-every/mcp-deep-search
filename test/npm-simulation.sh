...
timeout 1s node bin/mcp-deep-search.js 2>&1 | grep -q "deep-search MCP server running"
if [ $? -eq 0 ]; then
...
timeout 1s node bin/mcp-deep-search.js serve 2>&1 | grep -q "deep-search MCP server running"
if [ $? -eq 0 ]; then
...
node bin/mcp-deep-search.js capture --help > /dev/null 2>&1
if [ $? -eq 0 ]; then
